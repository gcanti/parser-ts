...
const getSemigroup = <I>(): SE.Semigroup<ParseError<I>> => ({
  concat: (x, y) => {
    if (x.input.cursor < y.input.cursor) return SE.last<ParseError<I>>().concat(x, y)
    if (x.input.cursor > y.input.cursor) return SE.first<ParseError<I>>().concat(x, y)

    return SE.struct<ParseError<I>>({
      input: SE.first<Stream<I>>(),
      fatal: SE.first<boolean>(),
