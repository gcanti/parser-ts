...
export function oneOf<F>(
  F: Functor<F> & Foldable<F>,
): (ss: HKT<F, string>) => P.Parser<C.Char, string>;
export function oneOf<F>(
  F: Functor<F> & Foldable<F>,
): (ss: HKT<F, string>) => P.Parser<C.Char, string> {
  return (ss) =>
...
export const fold: <I>(as: Array<P.Parser<I, string>>) => P.Parser<I, string> =
  M.fold(P.getMonoid(M.monoidString));
 ...
 * @since 0.6.0
 */
export const maybe: <I>(p: P.Parser<I, string>) => P.Parser<I, string> = P
  .maybe(M.monoidString);
 ...
 */
export const many: (
  parser: P.Parser<C.Char, string>,
) => P.Parser<C.Char, string> = (parser) => maybe(many1(parser));
 ...
 * @since 0.6.0
 */
export const doubleQuotedString: P.Parser<string, string> = P.surroundedBy(
  C.char('"'),
)(
...
export function run(
  string: string,
): <A>(p: P.Parser<C.Char, A>) => PR.ParseResult<C.Char, A> {
