...
describe("Parser", () => {
  it("eof", () => {
    ...
    assert.deepStrictEqual(
      S.run("")(parser),
      success(undefined, stream([]), stream([])),
    );
    assert.deepStrictEqual(
      S.run("a")(parser),
      error(stream(["a"]), ["end of file"]),
    );
  });
  ...
  it("filter", () => {
    const parser = P.expected(
      P.filter((c: C.Char) => c !== "a")(P.item<C.Char>()),
      'anything except "a"',
    );
    assert.deepStrictEqual(
      S.run("a")(parser),
      error(stream(["a"]), ['anything except "a"']),
    );
    assert.deepStrictEqual(
      S.run("b")(parser),
      success("b", stream(["b"], 1), stream(["b"])),
  ...
  it("lookAhead", () => {
    ...
    assert.deepStrictEqual(
      S.run("ab")(parser),
      success("abb", stream(["a", "b"], 2), stream(["a", "b"])),
    );
  });
  ...
  it("takeUntil", () => {
    ...
    assert.deepStrictEqual(
      S.run("ab")(parser),
      success(["a", "b"], stream(["a", "b"], 2), stream(["a", "b"])),
    );
    assert.deepStrictEqual(
      S.run("abc")(parser),
  ...
  it("optional", () => {
    ...
    assert.deepStrictEqual(
      S.run("a")(parser),
      success(some("a"), stream(["a"], 1), stream(["a"])),
    );
    assert.deepStrictEqual(
      S.run("b")(parser),
      success(none, stream(["b"], 0), stream(["b"])),
    );
  });
